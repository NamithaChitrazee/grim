...
#------------------------------------------------------------------------------
    def rename_json_filename(self,job):
                     ...
                     if (len(dbn.split('.')) == 1) :
                        ...
                        if (rc == 0):
                           for fn in glob.glob(sd2+'/*.'+ext) :
                               ...
                               nextkey = "file_name"
                               #os.chmod(fn, full_access_permission)
                               jsonfilename = os.path.basename(fn)
                               filename_without_extension = os.path.splitext(os.path.basename(jsonfilename))[0]
                               #print(jsonfilename, filename_without_extension)
                               with open(fn, 'r+') as file:
                                   json_data = json.load(file)
                                 
                               print("json data: ",json_data[nextkey])
                               # Modify the value of the key to the filename
                               json_data[nextkey] = filename_without_extension
                               print("json data: new filename: ",json_data[nextkey])
                               # Save the modified data to a new file in the same location
                               output_file_path = fn.replace('.json', '.modified.art.json')
